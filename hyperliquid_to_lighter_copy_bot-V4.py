#!/usr/bin/env python3
"""
Hyperliquid→Lighter Copy Trading Bot V3 - Educational Example
...
This bot monitors fills from a trader on Hyperliquid and mirrors them on Lighter.
Based on Copy-Lighter-V_1_9_3.py with Hyperliquid source monitoring.

V3 Features:
- Position tagging via client_order_index to support multiple bots on same account
- Only closes orphan positions opened by THIS bot (BOT_TAG = 2_000_000)
"""
...
import websocket  # For Hyperliquid (sync WebSocket)
import websockets  # For Lighter (async WebSocket)

# Hyperliquid SDK (for monitoring target trader)
...
class CopyTradingBot:
    """
    Copy trading bot that monitors a Hyperliquid trader and mirrors on Lighter
    
    Based on Copy-Lighter-V_1_9_3.py with Hyperliquid monitoring instead of Lighter→Lighter
    """
    ...
    def __init__(self):
        ...
        # === SOURCE: Hyperliquid Configuration ===
        self.hyperliquid_ws_url = os.getenv('HYPERLIQUID_WS_URL', 'wss://api.hyperliquid.xyz/ws')
        self.hyperliquid_target_wallet = os.getenv('HYPERLIQUID_TARGET_WALLET', '').strip().lower()
        
        # === DESTINATION: Lighter Configuration ===
        ...
        # Target is now on Hyperliquid (not Lighter account_index)
        ...
        self.target_positions = {}        # {coin: net size} - target trader on HYPERLIQUID (uses coins, not market_index)
        self.coin_to_market = {}          # NEW: Hyperliquid coin -> Lighter market_index mapping
        ...
        # NOTE: Target tracking uses COINS (Hyperliquid), our tracking uses market_index (Lighter)
        self.target_entry_prices = {}  # {coin: weighted_avg_entry_price} - using Hyperliquid coin names
        self.target_entry_costs = {}   # {coin: total_cost}
        self.target_position_sizes = {}  # {coin: total_size}
        self.target_exit_costs = {}    # {coin: total_exit_cost} - for PnL calculation
        ...
        self.BOT_TAG = 2_000_000  # This is the Hyperliquid→Lighter bot
        ...
        # Basic checks
        if not self.hyperliquid_target_wallet:
            print("\n❌ ERROR: HYPERLIQUID_TARGET_WALLET not set in .env\n")
            sys.exit(1)

        if not self.dry_run:
            if not self.api_private_key:
                print("\n❌ ERROR: Live mode requires API_PRIVATE_KEY in .env\n")
                sys.exit(1)
            if self.our_account_index is None or self.our_account_index == 0:
                print("\n❌ ERROR: Live mode requires OUR_ACCOUNT_INDEX in .env\n")
                sys.exit(1)
        ...
        # === Initialize Hyperliquid Info (read-only for target monitoring) ===
        self.hyperliquid_info = Info(constants.MAINNET_API_URL, skip_ws=True)

        print(f"\n🎯 SOURCE: Hyperliquid trader {self.hyperliquid_target_wallet}")
        print(f"🎯 DESTINATION: Lighter account {self.our_account_index if not self.dry_run else '(dry-run)'}")
    ...
    async def _initialize_lighter_sdk(self):
        ...
        try:
            ...
            # Initialize signer client for transactions (only if NOT dry run)
            if not self.dry_run:
                ...
                print("🧪 Dry run mode: Skipping SignerClient initialization")
            
            # Initialize API instances
            self.account_api = lighter.AccountApi(self.api_client)
            self.order_api = lighter.OrderApi(self.api_client)

            # Fetch account info (only if not dry run and we have account_index)
        ...
        try:
            ...
            if order_books and hasattr(order_books, 'order_books'):
                for ob in order_books.order_books:
                    ...
                    # NEW: Create coin→market mapping for Hyperliquid coins
                    self.coin_to_market[symbol.upper()] = market_index
                    
                print(f"✅ Loaded metadata for {len(self.market_metadata)} markets")
                if self.coin_to_market:
                    sample = dict(list(self.coin_to_market.items())[:5])
                    print(f"📊 Coin→Market mapping sample: {sample}")
    ...
    def _coin_to_market_index(self, coin):
        """Convert Hyperliquid coin to Lighter market_index
        
        Hyperliquid uses coin names like "BTC", "ETH", or "xyz:TOKEN" for HIP-3
        Lighter uses market_index integers like 0, 5, 8
        """
        # Handle HIP-3 format (e.g., "xyz:XYZ100")
        if ':' in coin:
            coin = coin.split(':')[1]
        
        # Try direct mapping
        market_index = self.coin_to_market.get(coin.upper())
        if market_index is not None:
            return market_index
        
        # Not found
        # print(f"⚠️  No market mapping found for Hyperliquid coin: {coin}")
        return None
    ...
    async def _sync_target_actual_positions_async(self):
        """Sync target trader's actual positions from Hyperliquid (async version)"""
        # This runs in async context but calls sync hyperliquid_info
        try:
            # Run the sync call in executor to not block event loop
            loop = asyncio.get_event_loop()
            user_state = await loop.run_in_executor(
                None,
                self.hyperliquid_info.user_state,
                self.hyperliquid_target_wallet
            )
            
            new_target_positions = {}
            
            # Hyperliquid user_state has 'assetPositions'
            if user_state and 'assetPositions' in user_state:
                for asset_pos in user_state['assetPositions']:
                    ...
                    if coin and abs(net_size) > 1e-8:
                        new_target_positions[coin] = net_size
    ...
    def _check_orphan_positions(self):
        """
        V3: Check orphan positions but ONLY close positions opened by THIS bot.
        ...
        # Get positions
        with self.state_lock:
            target_actual = dict(self.target_positions_actual)  # {coin: size} from Hyperliquid
            our_positions = dict(self.open_positions)  # {market_index: size} from Lighter
            position_tags = dict(self.position_tags)  # {market_index: client_order_index}
        ...
        for market_index, our_size in our_positions.items():
            ...
            # V3: Check if this position belongs to THIS bot
            ...
            if tag < self.BOT_TAG or tag >= self.BOT_TAG + 1_000_000:
                ...
                # (could be from bot 1, or manual trade, or bot with different tag)
                continue
            
            # Get coin name for this market_index
            symbol = self._get_market_symbol(market_index)
            
            # Check target's position for this coin (using symbol as coin name)
            target_size = target_actual.get(symbol, 0.0)
    ...
    def _coalesce_add_fill(self, fill_dict):
        """Add fill to coalescing buffer (for Hyperliquid fills)"""
        # Hyperliquid uses 'coin' instead of 'market_id'
        coin = fill_dict.get('coin', '')
        if not coin:
            return
        
        # Convert to market_index for internal use
        market_index = self._coin_to_market_index(coin)
        if market_index is None:
            return

        # Hyperliquid uses 'side': 'B' or 'A'
        hl_side = fill_dict.get('side', '')
        if hl_side == 'B':
            side = 'BUY'
        elif hl_side == 'A':
            side = 'SELL'
        ...
        with self.coalesce_lock:
            ...
            if key not in self._coalesce_buf:
                self._coalesce_buf[key] = {
                    ...
                    'coin': coin  # Store original coin for later
                }

            buf = self._coalesce_buf[key]
            # Hyperliquid uses 'sz' and 'px'
            size = float(fill_dict.get('sz', 0))
            price = float(fill_dict.get('px', 0))
    ...
    def _emit_aggregated_fill(self, key, buf):
        """Emit aggregated fill to processing queue (Hyperliquid format)"""
        self._agg_counter += 1
        ...
        # Get original coin from buffer
        coin = buf.get('coin', '')
        
        # Convert side back to Hyperliquid format
        hl_side = 'B' if side == 'BUY' else 'A'
        
        # Use Hyperliquid fill format
        agg_fill = {
            '_aggregated_id': agg_id,
            'coin': coin,
            'sz': str(buf['sum_sz']),
            'px': str(avg_price),
            'side': hl_side,
            'time': buf['max_time'],
            'dir': '',  # Will be determined in _process_target_fill
            '_is_aggregated': True,
    ...
    def _process_target_fill(self, fill):
        """Process a target trader fill from Hyperliquid"""
        try:
            # Hyperliquid fills use 'hash' and 'tid' instead of 'trade_id'
            # Can also have aggregated fills with '_aggregated_id'
            agg_id = fill.get('_aggregated_id')
            if agg_id:
                trade_id = agg_id
            else:
                trade_id = f"{fill.get('hash', '')}_{fill.get('tid', '')}"
            ...
            # NEW: Convert Hyperliquid coin to Lighter market_index
            coin = fill.get('coin', '')
            if not coin:
                return
            ...
            market_index = self._coin_to_market_index(coin)
            if market_index is None:
                return  # Coin not available on Lighter

            symbol = self._get_market_symbol(market_index)
            
            # Check coin filter (using symbol from Lighter)
            if self.coin_filter_mode == 'ENABLED':
                if self.enabled_coins and symbol not in self.enabled_coins:
                    return

            # Hyperliquid fills use 'sz' and 'px' instead of 'size' and 'price'
            ...
            event_ms = int(fill.get('time', 0))  # Hyperliquid uses 'time' not 'timestamp'
            ...
            # Target positions tracked by COIN (Hyperliquid naming) not market_index
            with self.state_lock:
                current_target = self.target_positions.get(coin, 0.0)
                delta = size if is_buy else -size
                new_target = current_target + delta
                self.target_positions[coin] = new_target
                
                # Track target's PnL for comparison (using COIN not market_index)
                ...
                if is_target_opening:
                    ...
                    old_cost = self.target_entry_costs.get(coin, 0)
                    new_cost = old_cost + (size * price)
                    
                    self.target_entry_costs[coin] = new_cost
                    self.target_position_sizes[coin] = abs(new_target)
                    if abs(new_target) > 0:
                        self.target_entry_prices[coin] = new_cost / abs(new_target)
                
                elif is_target_closing:
                    # Closing - track exit cost for PnL calculation (using COIN)
                    if not hasattr(self, 'target_exit_costs'):
                        self.target_exit_costs = {}
                    
                    old_exit_cost = self.target_exit_costs.get(coin, 0)
                    self.target_exit_costs[coin] = old_exit_cost + (size * price)
    ...
    async def _place_order_async(self, market_index, is_buy, size, price, is_closing, symbol, side_str):
        ...
        try:
            ...
            # V3: Generate tagged client_order_index for position tracking
            with self.order_counter_lock:
                self.order_counter += 1
                # BOT_TAG (2_000_000) + counter ensures uniqueness per bot
            ...
            async def _create_order_with_lock():
                ...
                try:
                    result = await self.signer_client.create_order(
                        ...
                        order_expiry=self.signer_client.DEFAULT_IOC_EXPIRY,
                        client_order_index=tagged_client_order_index  # V3: Tagged with BOT_TAG
                    )
                    return result
                finally:
                    self.order_lock.release()
            ...
            if err:
                ...
                # V3: Store the tag for this position (only for opens, not closes)
                if not is_closing:
                    with self.state_lock:
    ...
    def _apply_our_fill_to_positions(self, fill):
        ...
        4. V3: Extract and store client_order_index for position tagging
        """
        try:
            ...
            # V3: Extract client_order_index to identify which bot opened this position
    ...
    def _add_fill_to_buffer(self, market_index, delta, price, side, fill, client_order_index):
        """Add fill to buffer and start/reset timer for coalescing (V3: with tag tracking)"""
        with self.state_lock:
            if market_index not in self.pending_fills:
                self.pending_fills[market_index] = {
                    ...
                    'client_order_index': client_order_index  # V3: Store tag
                }
            ...
            # V3: Update client_order_index if this fill has one
    ...
    def _flush_pending_fills(self, market_index):
        ...
        with self.state_lock:
            ...
            side = buffer['side']
            fills = buffer['fills']
            client_order_index = buffer.get('client_order_index', 0)  # V3: Extract tag
            ...
            if is_opening:
                ...
                # V3: Store the tag for this position (for orphan detection)
                if client_order_index > 0:
            ...
            if abs(new_pos) < 1e-8:
                ...
                self.position_tags.pop(market_index, None)  # V3: Clean up tag
    ...
    # WebSocket Streams
    # ------------------------
    def stream_hyperliquid_ws(self):
        """Stream Hyperliquid WebSocket to monitor target trader (runs in separate thread)"""
        while not self.stop_event.is_set():
            try:
                if self.disconnect_start_time is None:
                    print(f"🔌 Connecting to Hyperliquid WebSocket...")
                ws = websocket.create_connection(self.hyperliquid_ws_url, timeout=30)
                
                # Subscribe to target wallet fills
                ws.send(json.dumps({
                    "method": "subscribe",
                    "subscription": {"type": "userFills", "user": self.hyperliquid_target_wallet}
                }))
                
                print("✅ Hyperliquid WebSocket connected. Listening for target fills...")
                if self.disconnect_start_time is not None:
                    elapsed = time.time() - self.disconnect_start_time
                    print(f"✅ HL reconnected after {elapsed:.0f}s outage")
                self.disconnect_start_time = None
                ...
                while not self.stop_event.is_set():
                    ...
                    if channel == "userFills" and isinstance(data, dict):
                        ...
                        if user == self.hyperliquid_target_wallet:
                            # Target trader's fills - add to coalescing buffer
                ...
                if self.disconnect_start_time is None:
                    ...
                    if code:
                        print(f"❌ Hyperliquid WebSocket error: HTTP {code}")
                    else:
                        print(f"❌ Hyperliquid WebSocket error: {e_str[:100]}")
                ...
                # Safety flatten check
                if self.safety_flatten_after_sec is not None:
                    elapsed = time.time() - self.disconnect_start_time
                    if elapsed >= self.safety_flatten_after_sec:
                        print(f"\n🚨 SAFETY: Disconnected for {elapsed:.1f}s. Consider flattening.\n")
                        self.disconnect_start_time = time.time()

                time.sleep(self.reconnect_delay_sec)

    async def stream_lighter_ws(self):
        """Stream Lighter WebSocket to track our own fills (optional, async)"""
        if self.dry_run:
            # In dry run, just sleep
            while not self.stop_event.is_set():
        ...
        while not self.stop_event.is_set():
            try:
                if not hasattr(self, '_lighter_ws_disconnect_time') or self._lighter_ws_disconnect_time is None:
                    print(f"🔌 Connecting to Lighter WebSocket (for our fills)...")
                ...
                async with websockets.connect(
                    ...
                    # Subscribe to our account fills
                    ...
                    while not self.stop_event.is_set():
                        ...
                        # Handle account updates (our fills)
                        if isinstance(msg, dict) and msg.get("type") == "update/account_all":
                            account = msg.get("account")
                            if account != self.our_account_index:
                                continue
                            ...
                            for market_index_str, trades in trades_by_market.items():
                ...
                if not hasattr(self, '_lighter_ws_disconnect_time') or self._lighter_ws_disconnect_time is None:
                    ...
                    code = next((c for c in ['500', '502', '503', '504'] if c in e_str), None)
                    if code:
                        print(f"❌ Lighter WebSocket error: HTTP {code} - reconnecting silently...")
                    else:
                        print(f"❌ Lighter WebSocket error: {e_str[:100]} - reconnecting silently...")
                    self._lighter_ws_disconnect_time = time.time()
    ...
    async def _run_async(self):
        ...
        if not self.dry_run:
            ...
            # Even in dry run, initialize SDK to fetch metadata for coin mapping
            await self._initialize_lighter_sdk()
            try:
                await self._fetch_market_metadata_async()
            except Exception as e:
        ...
        # Start Hyperliquid WebSocket in separate thread (sync)
        hyperliquid_ws_thread = threading.Thread(
            target=self.stream_hyperliquid_ws,
            name="hyperliquid-ws",
            daemon=True
        )
        hyperliquid_ws_thread.start()
        
        # Run Lighter WebSocket in this async context (for our fills)
        await self.stream_lighter_ws()
...
def main():
    if not os.path.exists('.env'):
        print("\n❌ No .env file found")
        print("📝 Setup: create .env file")
        print("   Required variables:")
        print("   - HYPERLIQUID_TARGET_WALLET=0x...")
        print("   - OUR_ACCOUNT_INDEX=...")
        print("   - API_PRIVATE_KEY=...")
        print("   - API_KEY_INDEX=3")
        print("   - DRY_RUN=true\n")
        sys.exit(1)
